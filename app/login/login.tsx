...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost/core-api/login.php', {
        method: 'POST',
      ...
      alert("Could not connect to the PHP server. Check if XAMPP/WAMP is running.");
    }
  };
      ...
      <div className="bg-white p-8 rounded-[24px] shadow-sm w-full max-w-[420px] border border-slate-100">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#6366f1] focus:ring-[#6366f1]" id="remember" />
        ...
        <p className="text-center text-slate-500 text-sm mt-10">
          Don't have an account? <Link to="/signup" className="text-[#6366f1] font-bold hover:underline">Sign up</Link>
        </p>
      </div>
    </div>
  );
